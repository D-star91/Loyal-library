<x-libcatlayout>


    <div class="fasting-contant">
        <div class="container">
            <div class="row ">
                <div class="col-md-12">
                    <div class="card mb-3 me-1" >
                        <div class="card-header lb-title-h4">BFAM Pray</div>
                            <div class="row ">
                                <div class="col-md-2">
                                    <img src="/image/BFAM pray.jpeg" alt="..." class="img-lb"/>
                                </div>
                                <div class="col-md-8">
                                    <div class="card-body">
                                        <h5 class="lb-title-h5">BFAM Pray</h5>
                                        <h6 class="lb-title-h6" >Author : Unknown</h6>
                                        <a href="" class="lb-btn-ty">life</a><a href="" class="lb-btn-ty">prayer</a>
                                        <ul class="lb-ul-img">
                                            <li><i class="far fa-calendar-alt me-2"></i>Date</li>
                                            <li><i class="fas fa-users me-2"></i>viwe</li>
                                            <li><i class="fas fa-file-archive me-2"></i>File-size</li>
                                        </ul>
                                        <a href="" class="lb-btn-down"><i class="fas fa-file-download me-2"></i></i>Download Here</a>
                                        <a href="" class="lb-btn-down"><i class="fas fa-book-open me-2"></i>Read this book online</a>
                                    </div>
                                </div>
                            </div>
                    </div> 
                </div>
            </div>
            <div class="row">
                <div class="col-md-12">
                    <div class="book-title">
                        <h4>Content in the book</h4>
                        <h5>BFAM pray</h5>
                        <p>
                            ဆုတောင်းခြင်းအသက်တာကို ပိုမိုရင့်ကျက်စေရန် သင်ခန်းစာပုံစံဖြင့် ပုံဖော်ပေးထားပါသည်။
                        </p>
                        <P>သင်ခန်းစာတစ်ခုစီတိုင်းသည် ယေရှုခရစ်တော်၏ ဆုတောင်းခြင်းအသက်တာကို အခြေခံပြီး လေ့လာရမှာပဲဖြစ်ပါသည်။ 
                            သင်ခန်းစာ အတွဲ (၄) တွဲ ပါဝင်ပြီး အတွဲတစ်တွဲလျှင် အခန်း (၁၃)ခန်းပါဝင်ပါသည်။
                        </P>
                        <P>ဤစာအုပ်၌ သင်ခန်းစာများကို လက်တွေ့လုပ်ဆောင်ခြင်းဖြင့် သင်၏ဆုတောင်းခြင်းအသက်တာကို ပိုမိုရင့်ကျက်လာစေမှာ အမှန်ပင်ဖြစ်ပါသည်။</P>
                    </div>
                </div>
                
            </div>
        </div>
    </div>

</x-libcatlayout>